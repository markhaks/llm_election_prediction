...
def compute_variant_metrics(
    ...
    # Korrelation der prognostizierten mit den tatsaechlichen Groessen.
    # Misst, ob die Rangfolge der Bundesstaaten erkannt wird, unabhaengig
    # von einer systematischen Verschiebung im Niveau (Bias).
    pearson_margin = valid[["pred_margin", "actual_margin"]].corr().iloc[0, 1]
    spearman_margin = valid[["pred_margin", "actual_margin"]].corr(method="spearman").iloc[0, 1]
    pearson_share = valid[["pred_rep_share", "actual_rep_share"]].corr().iloc[0, 1]

    # Fehler getrennt nach tatsaechlichem Gewinner. Bei einem gerichteten
    # Klassifikatorfehler zeigt sich hier ein deutlicher Unterschied:
    # In Harris-Staaten wird der republikanische Anteil zu hoch geschaetzt,
    # in Trump-Staaten wird der bereits hohe Anteil weniger stark ueberschaetzt.
    trump_states = valid[valid["actual_winner"] == "R"]
    harris_states = valid[valid["actual_winner"] == "D"]
    ...
    metrics = {
        ...
        "median_abs_fehler_spanne_pp": round(valid["abs_error_margin"].median() * 100, 2),
        "pearson_spanne": round(pearson_margin, 4),
        "spearman_spanne": round(spearman_margin, 4),
        "pearson_anteil": round(pearson_share, 4),
        "mae_spanne_trump_staaten_pp": round(trump_states["abs_error_margin"].mean() * 100, 2),
        "mae_spanne_harris_staaten_pp": round(harris_states["abs_error_margin"].mean() * 100, 2),
        "bias_spanne_trump_staaten_pp": round(trump_states["error_margin"].mean() * 100, 2),
        "bias_spanne_harris_staaten_pp": round(harris_states["error_margin"].mean() * 100, 2),
        "n_trump_staaten": len(trump_states),
        "n_harris_staaten": len(harris_states),
