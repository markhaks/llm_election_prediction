...
It reports:
- total matched rows from multiple valid geolocation folders
- rows not matched
- match rate
- tweets per state
- tweets per city
...
def aggregate_state_week_counts(files: list[Path]) -> pd.DataFrame:
    ...
    required_columns = ["tweet_date", "user_state", "user_state_name"]
...
def main() -> None:
    ...
    for matched_dir in MATCHED_DIRS:
        files = find_csv_files(matched_dir)
        matched_files.extend(files)

    not_matched_files = find_csv_files(NOT_MATCHED_DIR)

    print(f"Matched folders: {len(MATCHED_DIRS)}")
    print(f"Matched files total: {len(matched_files)}")
    ...
    summary_df = pd.DataFrame(
        [
            ...
            {"metric": "matched_folders", "value": len(MATCHED_DIRS)},
            {"metric": "matched_files_total", "value": len(matched_files)},
            {"metric": "not_matched_files_total", "value": len(not_matched_files)},
        ]
    )
