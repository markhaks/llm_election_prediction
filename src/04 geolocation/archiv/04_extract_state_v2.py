...
def extract_state_abbreviation(location: str) -> tuple[str | None, str | None]:
    """
    Conservative extraction of US state abbreviations.
    ...
    location_original = str(location).strip()

    # Case 1: exact abbreviation only, e.g. "TX"
    if location_original in US_STATES:
        return location_original, "state_abbreviation_exact"

    # Case 2: abbreviation after comma, e.g. "Austin, TX"
    comma_pattern = r",\s*([A-Z]{2})(?:\s|,|$)"
    matches = re.findall(comma_pattern, location_original)

    for state_abbr in matches:
        if state_abbr in US_STATES:
            if state_abbr in AMBIGUOUS_STATE_ABBR and not has_us_context(location_original):
                return None, None

            return state_abbr, "state_abbreviation_after_comma"

    # Case 3: abbreviation at the end, e.g. "Austin TX"
    ...
    if match:
        ...
        if state_abbr in US_STATES:
            ...
            return state_abbr, "state_abbreviation_at_end"
...
def extract_state(location) -> tuple[str | None, str | None]:
    ...
    # 2. Match abbreviations conservatively
    state_abbr, method = extract_state_abbreviation(location)

    if state_abbr:
        return state_abbr, method
...
def find_input_files(input_dir: Path) -> list[Path]:
    ...
    if not files:
        raise FileNotFoundError(
            f"No state-detected CSV files found in: {input_dir}"
        )
